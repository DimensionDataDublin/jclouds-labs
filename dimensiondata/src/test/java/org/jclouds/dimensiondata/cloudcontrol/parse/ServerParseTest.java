...
public class ServerParseTest extends BaseDimensionDataCloudControlParseTest<Server> {
   ...
   public Server expected() {
      ...
      return Server.builder().id("b8246ba4-847d-475b-b296-f76787a69ca8").name("parser-test-server-name")
            ...
            .createTime(parseDate("2016-03-10T13:05:21.000Z")).started(true).deployed(true).guest(Guest.builder()
                  ...
                  .vmTools(VmTools.builder().apiVersion(9354).type(VmTools.Type.VMWARE_TOOLS)
                        .versionStatus(VmTools.VersionStatus.NEED_UPGRADE).runningStatus(VmTools.RunningStatus.RUNNING)
                        .build()).osCustomization(true).build())
            .cpu(CPU.builder().count(2).speed("STANDARD").coresPerSocket(1).build()).memoryGb(4).ideControllers(
                  ImmutableList.of(IdeController.builder().id("5bdfc88c-5509-420c-8c5d-cc7876dacfbe")
                              .adapterType("IDE_CONTROLLER_XXX").channel(0).key(3000).state(State.NORMAL).build(),
                        IdeController.builder().id("def96a00-d1ee-48b9-b07d-3993594724d0")
                              .adapterType("IDE_CONTROLLER_XXX").channel(1).key(3001).deviceOrDisks(ideDisksOrDevices)
            ...
            .scsiControllers(ImmutableList
                  ...
                  NIC.builder().id("980a9fdd-4ea2-478b-85b4-f016349f1738").privateIpv4("10.0.0.8")
                        ...
                        .vlanName("jclouds vlan").state(State.NORMAL).build()).additionalNic(null)
