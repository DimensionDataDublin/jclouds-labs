...
public abstract class Hypervisor {
   ...
   public static Hypervisor create(String type, String maintenanceStatus, List<Property> properties,
         List<DiskSpeed> disksSpeed, List<CpuSpeed> cpusSpeed) {
      return builder().type(type).maintenanceStatus(maintenanceStatus).properties(properties).disksSpeed(disksSpeed)
            .cpusSpeed(cpusSpeed).build();
   }
